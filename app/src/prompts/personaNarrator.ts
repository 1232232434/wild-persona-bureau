...
> = {
  social: {
    label: '社交模式',
    high: '你会自然把周围的人拉进同一个节奏，也愿意主动把气氛带起来',
    low: '你会先观察和判断，再决定要不要靠近人群',
  },
  risk: {
    label: '冒险胆量',
    high: '事情越新、越快、越没标准答案，你越容易被激活',
    low: '你会先看清代价和退路，再决定这一步值不值得往前迈',
  },
  stress: {
    label: '压力反应',
...
- 性格底色：${input.archetype.personality}
- 爱情模式：${input.archetype.love}
- 事业学习：${input.archetype.career}
- 动物说明：${input.archetype.animalProfile.vibe}
...
- actionAdvice：给你一个现实可执行的建议，最好能明天就用上，可以贴近恋爱、学习或工作
...
export const createFallbackNarration = (input: PersonaNarrationInput): PersonaNarration => {
  ...
  return {
    opener: `${input.archetype.animal}型的你，越相处越能看出底色。`,
